...
namespace CubSub
{	
	public class PlaceHolderMenuItems : MonoBehaviour
	{
		/*
		...
		{	
			//GET DA PATH (to where we'll load the .dat from) (Namespaces For The Win) 
			...
			//GET DA ODDER PATH (to where we'll save the .vdb)
			string toVBS = EditorUtility.SaveFilePanel("Choose a location to save the VoxelDatabase (.vdb) file.", Paths.VDBFiles, "NewVDB", "vdb");

			if(toVBS.Length == 0) return;
